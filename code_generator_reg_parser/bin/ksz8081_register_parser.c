...
static bool parse_basic_control_register_0 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (reg16b_val & (1<<9)) {
        fprintf (out_file_prt,"\n    reg %02u bit  9:1  RW/SC Auto-Negotiation  Restart auto-negotiation process", reg_addr);
    }
    if (0==(reg16b_val & (1<<9))) {
...
static bool parse_phy_identifier_1_register_2 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    uint16_t code = (0xffff & (reg16b_val>>0)));
...
static bool parse_phy_identifier_2_register_3 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    uint16_t code = (0x3f & (reg16b_val>>10)));
    fprintf (out_file_prt,"\n    reg %02u bit 15-10: %u=0x%x  RO Assigned to the 19th through 24th bits of the Organizationally Unique Identifier (OUI). KENDIN Communication’s OUI is 0010A1 (hex).", reg_addr, code, code); 
    uint16_t code = (0x3f & (reg16b_val>>4)));
    fprintf (out_file_prt,"\n    reg %02u bit 9-4: %u=0x%x  RO Six-bit manufacturer’s model number", reg_addr, code, code); 
    uint16_t code = (0xf & (reg16b_val>>0)));
    fprintf (out_file_prt,"\n    reg %02u bit 3-0: %u=0x%x  RO Four-bit manufacturer’s revision number", reg_addr, code, code); 
    return res;
}
...
static bool parse_auto_negotiation_expansion_register_6 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    uint16_t code = (0x7ff & (reg16b_val>>5)));
    ...
    if (0==(reg16b_val & (1<<0))) {
        fprintf (out_file_prt,"\n    reg %02u bit  0:0  RO Link partner does not have auto-negotiation capability", reg_addr);
    }
    return res;
}
...
static bool parse_auto_negotiation_next_page_register_7 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (reg16b_val & (1<<11)) {
        fprintf (out_file_prt,"\n    reg %02u bit 11:1  RO Previous value of the transmitted link code word equaled logic 1", reg_addr);
    }
    if (0==(reg16b_val & (1<<11))) {
        fprintf (out_file_prt,"\n    reg %02u bit 11:0  RO Previous value of the transmitted link code word equaled logic 0", reg_addr);
    }
    uint16_t code = (0x7ff & (reg16b_val>>0)));
    fprintf (out_file_prt,"\n    reg %02u bit 10-0: %u=0x%x  RW 11-bit wide field to encode 2048 messages", reg_addr, code, code); 
    return res;
}
...
static bool parse_link_partner_next_page_ability_register_8 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    uint16_t code = (0x7ff & (reg16b_val>>0)));
    fprintf (out_file_prt,"\n    reg %02u bit 10-0: %u=0x%x  RO 11-bit wide field to encode 2048 messages", reg_addr, code, code); 
    return res;
}
...
static bool parse_rxer_counter_register_21 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    uint16_t code = (0xffff & (reg16b_val>>0)));
    fprintf (out_file_prt,"\n    reg %02u bit 15-0: %u=0x%x  RO/SC Receive error counter for symbol error frames", reg_addr, code, code); 
    return res;
}
...
static bool parse_operation_mode_strap_status_register_23 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    uint16_t code = (0x7 & (reg16b_val>>13)));
...
static bool parse_linkmd_control_status_register_29 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0==(reg16b_val & (1<<15))) {
        fprintf (out_file_prt,"\n    reg %02u bit 15:0  RW/SC Indicates cable diagnostic test (if enabled) has completed and the status information is valid for read.", reg_addr);
    }
    ...
    if (reg16b_val & (1<<12)) {
        fprintf (out_file_prt,"\n    reg %02u bit 12:1  RO Short cable (<10 meter) has been detected by LinkMD", reg_addr);
    }
    if (0==(reg16b_val & (1<<12))) {
...
static bool parse_phy_control_2_register_31 (uint16_t reg16b_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (reg16b_val & (1<<14)) {
        fprintf (out_file_prt,"\n    reg %02u bit 14:1  RW MDI-X mode Transmit on RXP, RXM (Pins 10, 9) and Receive on TXP, TXM (Pins 12, 11)", reg_addr);
    }
    if (0==(reg16b_val & (1<<14))) {
        fprintf (out_file_prt,"\n    reg %02u bit 14:0  RW MDI mode Transmit on TXP, TXM (Pins 12, 11) and Receive on RXP, RXM (Pins 10, 9)", reg_addr);
    }
    if (reg16b_val & (1<<13)) {
