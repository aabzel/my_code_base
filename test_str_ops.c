...
bool test_extract_sub_string(void) {
	...
	char inStr[100] = "";
	strncpy(inStr,
			"Device [TSTI_V2] Serial 0x%llx from IP %s MAC %s user <@aabdev>",
			sizeof(inStr));
...
bool test_detect_change(void) {
	...
	// "aaabb"
	// "aaa11bb"
	detect_change("aaabb", "aaa11bb", &oldSubString, &oldSubStringLen,
			&newSubString, &newSubStringLen);
	...
	detect_change("aaa11bb", "aaabb", &oldSubString, &oldSubStringLen,
			&newSubString, &newSubStringLen);
	...
	detect_change("aa111bb", "aa22bb", &oldSubString, &oldSubStringLen,
			&newSubString, &newSubStringLen);
...
bool test_myAtoi(void) {
	...
	if (val != atoi(inStr)) {
		printf("\n%s %d [%s] val %d exp val %d", __FUNCTION__, __COUNTER__,
				inStr, val, atoi(inStr));
		return false;
	}
	...
	val = myAtoi(inStr);
	if (4193 != val) {
		printf("\n%s %d [%s] outVal [%d]", __FUNCTION__, __COUNTER__, inStr,
				val);
		return false;
	}
	if (val != atoi(inStr)) {
	...
	val = myAtoi(inStr);
	if (-42 != val) {
		printf("\n%s %d [%s] val %d exp(%d)", __FUNCTION__, __COUNTER__, inStr,
				val, -42);
		return false;
	}
	if (val != atoi(inStr)) {
...
bool test_num_to_bin_str(void) {
	int cmpRes = 0;
	cmpRes = strcmp("1010_1010_1010_1010_1010_1010_1010_1010",
			uint32_to_bin_str(0xAAAAAAAA));
	if (0 != cmpRes) {
		return false;
	}
	cmpRes = strcmp("0101_0101_0101_0101_0101_0101_0101_0101",
			uint32_to_bin_str(0x55555555));
	if (0 != cmpRes) {
		return false;
	}
	cmpRes = strcmp("0000_0000_0000_0000_0000_0000_0000_0000",
			uint32_to_bin_str(0x00000000));
	if (0 != cmpRes) {
		return false;
	}
	cmpRes = strcmp("1111_1111_1111_1111_1111_1111_1111_1111",
			uint32_to_bin_str(0xFFFFFFFF));
	if (0 != cmpRes) {
		return false;
	}
	return true;
