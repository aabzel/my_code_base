...
"I","Xe","Cs","Ba","La","Ce","Pr","Nd","Pm","Sm","Eu","Gd","Tb","Dy","Ho","Er","Tm","Yb","Lu","Hf","Ta","W","Re","Os","Ir","Pt","Au","Hg","Tl","Pb","Bi","Po",
...
string countOfAtoms(string formula) {
     ...
     for (auto& p : m){
         curr+=p.first;
         if (p.second>1) curr+=to_string(p.second);
     }
     return curr;
 }
#endif
        	...
        	if (isalpha(formula[i])) {
        		...
        		if(1<=stack_stack.size()) {
        			cout << "        stack_stack contain ["<< stack_stack.top() <<"]"<< endl;
             		temp_str = stack_stack.top();stack_stack.pop();
                ...
                cout << "     expand_element " << expand_element << endl;
                cur_str = expand_element;

                if (0==round_bracket_open) {
                	temp_res = expand_element;
                } else {
