	...
	for (i = 0; i < MAX_AMOUNT_OF_DEVICES; i++) {
		if (true == deviceList[i].isbusy) {
			printf(
					...
					deviceList[i].holderMac[0], deviceList[i].holderMac[1],
					deviceList[i].holderMac[2], deviceList[i].holderMac[3],
					deviceList[i].holderMac[4], deviceList[i].holderMac[5],
					ip_to_str((uint8_t*) &deviceList[i].holderIp),
					deviceList[i].holderName, deviceList[i].spotTimeStr);
			res = true;
		}
	}
	printf("\n");
...
static bool save_device_in_log_file(char *fileName, xConnection_t inDev){
    ...
    if(NULL!=filePtr) {
    	fprintf(filePtr,
				"\n%s Serial [0x%016llx] holder MAC [%02X:%02X:%02X:%02X:%02X:%02X] IP %s name %s spot %s",
				inDev.deviceName,
				(long long unsigned int) inDev.serialNumber,
				inDev.holderMac[0], inDev.holderMac[1],
				inDev.holderMac[2], inDev.holderMac[3],
				inDev.holderMac[4], inDev.holderMac[5],
				ip_to_str((uint8_t*) &inDev.holderIp),
				inDev.holderName, inDev.spotTimeStr);
...
static bool update_dev_table(char *rxArray, uint16_t rxByteCnt) {
	...
	if ((NULL != rxArray) && (1 < rxByteCnt)) {
		...
		time_t t = time(NULL);
		struct tm tm = *localtime(&t);
		snprintf(newDevice.spotTimeStr, sizeof(newDevice.spotTimeStr),
...
bool launch_tcp_server(uint16_t tcpServerPort) {
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
		...
		//Create a socket
		if ((sockeDescriptor = socket(AF_INET, SOCK_STREAM, 0))
				== INVALID_SOCKET) {
			...
			//Bind
			if (bind(sockeDescriptor, (struct sockaddr*) &server,
				...
				new_socket = accept(sockeDescriptor, (struct sockaddr*) &client,
						&addrlen);
				while (new_socket != INVALID_SOCKET) {
					...
					new_socket = accept(sockeDescriptor,
