...
bool parse_tic12400_regs_file (char *inFileName, char *outFileName) {
    ...
    if (outFilePrt) {
        if (true == res) {
            ...
            printf ("\n processed %u/47 registers %f%% remain %u registers", procRegCnd,
...
char *tic12400_reg_2_name[51] = {
    "   REG_RESERVED_00H", // 0
    "      REG_DEVICE_ID", "       REG_INT_STAT", "            REG_CRC", "   REG_IN_STAT_MISC", "   REG_IN_STAT_COMP",
    "   REG_IN_STAT_ADC0", "   REG_IN_STAT_ADC1", "REG_IN_STAT_MATRIX0", "REG_IN_STAT_MATRIX1",
    "      REG_ANA_STAT0", // 10
    "      REG_ANA_STAT1", "      REG_ANA_STAT2", "      REG_ANA_STAT3", "      REG_ANA_STAT4", "      REG_ANA_STAT5",
    "      REG_ANA_STAT6", "      REG_ANA_STAT7", "      REG_ANA_STAT8", "      REG_ANA_STAT9",
    "     REG_ANA_STAT10", // 20
    "     REG_ANA_STAT11", "     REG_ANA_STAT12", "   REG_RESERVED_17H", "   REG_RESERVED_18H", "   REG_RESERVED_19H",
    "         REG_CONFIG", "         REG_IN_EN",  "     REG_CS_SELECT",  "       REG_WC_CFG0",
    "       REG_WC_CFG1", // 30
    "      REG_CCP_CFG0",  "      REG_CCP_CFG1",  "    REG_THRES_COMP",  "  REG_INT_EN_COMP1",  "  REG_INT_EN_COMP2",
    "   REG_INT_EN_CFG0",  "   REG_INT_EN_CFG1",  "   REG_INT_EN_CFG2",  "   REG_INT_EN_CFG3",
    "   REG_INT_EN_CFG4", // 40
    "    REG_THRES_CFG0",  "    REG_THRES_CFG1",  "    REG_THRES_CFG2",  "    REG_THRES_CFG3",  "    REG_THRES_CFG4",
    " REG_THRESMAP_CFG0",  " REG_THRESMAP_CFG1",  " REG_THRESMAP_CFG2",  "        REG_MATRIX",
    "          REG_MODE" // 50
};
...
bool parse_tic12400_reg (uint8_t regAddr, FILE *outFilePrt) {
    ...
    tic12400RegMap[regAddr].reg24BitVal = regVal;
    fprintf (outFilePrt, "\n\nreg %s addr 0x%02x val 0x%06x 0b_%s", tic12400_reg_2_name[regAddr], regAddr, regVal,
             utoa_bin24 (regVal));
    switch (regAddr) {
    case REG_IN_STAT_MATRIX0:
        procRegCnd++;
        res = parse_tic12400_in_stat_matrix0_register_08h (regVal, outFilePrt);
        break;
    case REG_IN_STAT_MATRIX1:
        procRegCnd++;
        res = parse_tic12400_in_stat_matrix1_register_09h (regVal, outFilePrt);
        break;
    ...
    default:
#if DPLOY_LACK
        fprintf (outFilePrt, "\n Lack of parser for reg %s addr 0x%x val 0x%x", tic12400_reg_2_name[regAddr], regAddr,
                 regVal);
#endif
        break;
    }
...
bool parse_tic12400_interrupt_status_register_02h (uint32_t regVal, FILE *outFilePrt) {
    ...
    if (regVal & INT_STAT_CHK_FAIL_13) {
        fprintf (outFilePrt, "\n bit 13:1 RC An error is detected when loading factory settings into the device upon "
...
bool parse_tic12400_wc_cfg1_register_1eh (uint32_t regVal, FILE *outFilePrt) {
    ...
    if (regVal & WC_CFG1_AUTO_SCALE_DIS_CSI_22) {
        fprintf (outFilePrt, "\n  bit  22:1 RW Disable wetting current auto-scaling (to 2mA) in continuous mode for "
                             "10mA and 15mA settings upon switch closure for all inputs enabled with CS");
    } else {
        fprintf (outFilePrt, "\n  bit  22:0 RW Enable wetting current auto-scaling (to 2mA) in continuous mode for "
                             "10mA and 15mA settings upon switch closure for all inputs enabled with CSI");
    }

    if (regVal & WC_CFG1_AUTO_SCALE_DIS_CSO_21) {
        fprintf (outFilePrt, "\n  bit  21:1 RW Disable wetting current auto-scaling (to 2mA) in continuous mode for "
                             "10mA and 15mA settings upon switch closure for all inputs enabled with CSO");
    } else {
        fprintf (outFilePrt, "\n  bit  21:0 RW Enable wetting current auto-scaling (to 2mA) in continuous mode for "
                             "10mA and 15mA settings upon switch closure for all inputs enabled with CSO");
    }
...
bool parse_tic12400_in_stat_matrix0_register_08h (uint32_t regVal, FILE *outFilePrt) {
    ...
    if (NULL != outFilePrt) {
        ...
        for (bit = 23; 0 <= bit; bit--) {
            if (regVal & (1 << bit)) {
                fprintf (outFilePrt, "\n  bit %2u:1 R Input IN%02u is above threshold while IN%u pulled to GND", bit,
                         inStatMatrix0InChanLUT[bit], inStatMatrix0InChan2LUT[bit]);
            } else {
                fprintf (outFilePrt, "\n  bit %2u:0 R Input IN%02u is below threshold while IN%u pulled to GND", bit,
                         inStatMatrix0InChanLUT[bit], inStatMatrix0InChan2LUT[bit]);
            }
        }
        res = true;
...
bool parse_tic12400_in_stat_matrix1_register_09h (uint32_t regVal, FILE *outFilePrt) {
    ...
    if (NULL != outFilePrt) {
        ...
        for (bit = 23; 11 <= bit; bit--) {
            if (regVal & (1 << bit)) {
                fprintf (outFilePrt, "\n  bit %2u:1 R Input IN%02u equal to or above threshold THRES_COM", bit,
                         inStatMatrix1InChanLUT[bit]);
            } else {
                fprintf (outFilePrt, "\n  bit %2u:0 R Input IN%02u below threshold THRES_COM", bit,
                         inStatMatrix1InChanLUT[bit]);
            }
        }
...
bool parse_tic12400_config_register_1ah (uint32_t regVal, FILE *outFilePrt) {
    ...
    if (regVal & TW_CUR_DIS_CSI_16) {
        fprintf (outFilePrt, "\n  bit 16: Disable wetting current reduction (to 2 mA) for 10mA and 15mA settings upon "
                             "TW event for all inputs enabled with CSI.");
    } else {
        fprintf (outFilePrt, "\n  bit 16: Enable wetting current reduction (to 2 mA) for 10mA and 15mA settings upon "
                             "TW event for all inputs enabled with CSI");
    }

    if (regVal & TW_CUR_DIS_CSO_13) {
        fprintf (outFilePrt, "\n  bit 13: RW  Disable wetting current reduction (to 2mA) for 10mA and 15mA settings "
                             "upon TW event for all inputs enabled with CSO");
    } else {
        fprintf (outFilePrt, "\n  bit 13: RW  Enable wetting current reduction (to 2mA) for 10mA and 15mA settings "
                             "upon TW event for all inputs enabled with CSO");
    }
...
bool parse_tic12400_in_stat_adc1_register_07h (uint32_t regVal, FILE *outFilePrt) {
    ...
    reserved = extract_subval_from_32bit (regVal, 23, 13);
    fprintf (outFilePrt, "\n  bit 23-13:%5u R Reserved", reserved);

    ina_23 = extract_subval_from_32bit (regVal, 12, 10);
    parse_in_a_code (ina_23, outFilePrt, 12, 10, 23);

    ina_22 = extract_subval_from_32bit (regVal, 9, 8);
    parse_in_a_code (ina_22, outFilePrt, 9, 8, 22);

    ina_21 = extract_subval_from_32bit (regVal, 7, 6);
    parse_in_a_code (ina_21, outFilePrt, 7, 6, 21);

    ina_20 = extract_subval_from_32bit (regVal, 5, 4);
    parse_in_a_code (ina_20, outFilePrt, 5, 4, 20);

    ina_19 = extract_subval_from_32bit (regVal, 3, 2);
    parse_in_a_code (ina_19, outFilePrt, 3, 2, 19);

    ina_18 = extract_subval_from_32bit (regVal, 1, 0);
    parse_in_a_code (ina_18, outFilePrt, 1, 0, 18);
...
bool parse_tic12400_in_en_bit (uint32_t regVal, uint8_t bitNum, FILE *outFilePrt) {
    ...
    if (regVal & (1U << bitNum)) {
        ...
        fprintf (outFilePrt, "\n  bit %02u: Input channel IN%u disabled. Polling sequence skips this channel", bitNum,
                 bitNum);
    }
    return res;
}
...
static bool parse_ccp_time (uint8_t ccp_time, FILE *outFilePrt) {
    bool res = false;
    if (NULL != outFilePrt) {
        res = true;
        switch (ccp_time) {
        case 0:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 64 us",
                     ccp_time);
            break;
        case 1:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 128 us",
                     ccp_time);
            break;
        case 2:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 192 us",
                     ccp_time);
            break;
        case 3:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 256 us",
                     ccp_time);
            break;
        case 4:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 320 us",
                     ccp_time);
            break;
        case 5:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 384 us",
                     ccp_time);
            break;
        case 6:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 448 us",
                     ccp_time);
            break;
        case 7:
            fprintf (outFilePrt,
                     "\n  bit 6-4:%u RW Wetting current activation time in CCP (Clean Current Polling) mode 512 us",
                     ccp_time);
            break;
        default:
            res = false;
            fprintf (outFilePrt, "\n  bit 6-4:%u RW UNDEF VALUE ERROR!", ccp_time);
            break;
        }
    }
    return res;
...
bool parse_tic12400_ccp_cfg0_register_1fh (uint32_t regVal, FILE *outFilePrt) {
    ...
    (void)outFilePrt;
    uint32_t reserved;
    reserved = extract_subval_from_32bit (regVal, 23, 7);
    fprintf (outFilePrt, "\n  bit 23-7:%u R Reserved", reserved);
    uint32_t ccp_time;
    ccp_time = extract_subval_from_32bit (regVal, 6, 4);
    parse_ccp_time (ccp_time, outFilePrt);

    if (regVal & CCP_CFG0_WC_CCP3) {
        fprintf (outFilePrt,
                 "\n  bit 3:1 Wetting current setting for IN18 to IN23 in CCP mode 15mA (typ.) wetting current");
    } else {
        fprintf (outFilePrt,
                 "\n  bit 3:0 Wetting current setting for IN18 to IN23 in CCP mode 10mA (typ.) wetting current");
    };
    if (regVal & CCP_CFG0_WC_CCP2) {
        fprintf (outFilePrt,
                 "\n  bit 2:1 Wetting current setting for IN12 to IN17 in CCP mode 15mA (typ.) wetting current");
    } else {
        fprintf (outFilePrt,
                 "\n  bit 2:0 Wetting current setting for IN12 to IN17 in CCP mode 10mA (typ.) wetting current");
    };
    if (regVal & CCP_CFG0_WC_CCP1) {
        fprintf (outFilePrt,
                 "\n  bit 1:1 Wetting current setting for IN6 to IN11 in CCP mode 15mA (typ.) wetting current");
    } else {
        fprintf (outFilePrt,
                 "\n  bit 1:0 Wetting current setting for IN6 to IN11 in CCP mode 10mA (typ.) wetting current");
    };
    if (regVal & CCP_CFG0_WC_CCP0) {
        fprintf (outFilePrt,
                 "\n  bit 0:1 Wetting current setting for IN0 to IN5 in CCP mode 15mA (typ.) wetting current");
    } else {
        fprintf (outFilePrt,
...
bool parse_interrupt_en_comp_mode (uint8_t reg_val, FILE *outFilePrt, uint8_t oldBit, uint8_t littleBit,
    ...
    switch (reg_val) {
    case 0:
        fprintf (outFilePrt, "\n  bit %02u-%02u:%u R/W no interrupt generation for IN%u", oldBit, littleBit, reg_val,
                 channel);
        break;
    case 1:
        fprintf (outFilePrt, "\n  bit %02u-%02u:%u R/W interrupt generation on rising edge for IN%u", oldBit, littleBit,
                 reg_val, channel);
        break;
    case 2:
        fprintf (outFilePrt, "\n  bit %02u-%02u:%u R/W interrupt generation on falling edge for IN%u", oldBit,
    ...
    if (channel <= 17) {
        if (0 < (oldBit - littleBit)) {
            ...
            case 0:
                ...
                fprintf (outFilePrt, "\n  bit %02u-%02u:%u R Input IN%u is below threshold 2A.", oldBit, littleBit,
                         ina_code, channel);
                break;
            case 1:
                snprintf (tic12400channelList[channel].inputADCLevel,
            ...
            case 2:
                ...
                fprintf (outFilePrt, "\n  bit %02u-%02u:%u R Input IN%u is equal to or above threshold 2B", oldBit,
            ...
            if ((oldBit == littleBit) && (littleBit <= 11)) {
                if (1 == ina_code) {
                    ...
                    fprintf (outFilePrt, "\n  bit    %02u:%u R Input IN%02u is above configured threshold", littleBit,
                             ina_code, channel);
                }
                if (0 == ina_code) {
                    res = true;
                    snprintf (tic12400channelList[channel].inputADCLevel,
                              sizeof (tic12400channelList[channel].inputADCLevel), "below");
                    fprintf (outFilePrt, "\n  bit    %02u:%u R Input IN%02u is below configured threshold", littleBit,
                             ina_code, channel);
                }
            }
            return res;
        ...
        case 0:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "below");
            fprintf (outFilePrt, "\n  bit %02u-%02u:%u R Input IN%u is below threshold 3A.", oldBit, littleBit,
                     ina_code, channel);
            break;
        case 1:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "middle");
            fprintf (outFilePrt,
                     "\n  bit %02u-%02u:%u R Input IN%u is below threshold 3B and equal to or above threshold 3A",
                     oldBit, littleBit, ina_code, channel);
            break;
        case 2:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "edge");
            fprintf (outFilePrt,
                     "\n  bit %02u-%02u:%u R Input IN%u is below threshold 3C and equal to or above threshold 3B",
                     oldBit, littleBit, ina_code, channel);
            break;
        case 3:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "N/A");
            fprintf (outFilePrt, "\n  bit %02u-%02u:%u R Input is IN%u is equal to or above threshold 3C", oldBit,
                     littleBit, ina_code, channel);
            break;
        default:
            res = false;
        ...
        case 0:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "below");
            fprintf (outFilePrt, "\n  bit %02u-%02u:%u Input IN23 is below threshold 3A.", oldBit, littleBit, ina_code);
            break;
        case 1:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "mid");
            fprintf (outFilePrt,
                     "\n  bit %02u-%02u:%u R Input IN23 is below threshold 3B and equal to or above threshold 3A.",
        ...
        case 2:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "mid");
            fprintf (outFilePrt,
                     "\n  bit %02u-%02u:%u R Input IN23 is below threshold 3C and equal to or above threshold 3B.",
                     oldBit, littleBit, ina_code);
            break;
        case 3:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "mid");
            fprintf (outFilePrt,
                     "\n  bit %02u-%02u:%u R Input IN23 is below threshold THRES8 and equal to or above threshold 3C.",
                     oldBit, littleBit, ina_code);
            break;
        case 4:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
            ...
            fprintf (
                outFilePrt,
                "\n  bit %02u-%02u:%u R Input IN23 is below threshold THRES9 and equal to or above threshold THRES8.",
                oldBit, littleBit, ina_code);
            break;
        case 5:
            snprintf (tic12400channelList[channel].inputADCLevel, sizeof (tic12400channelList[channel].inputADCLevel),
                      "above");
            fprintf (outFilePrt, "\n  bit %02u-%02u:%u R Input IN23 is equal to or above threshold THRES9", oldBit,
                     littleBit, ina_code);
            break;
        default:
            res = false;
