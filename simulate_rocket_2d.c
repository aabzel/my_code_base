...
bool simulate_roket (float runTime) {
    ...
    if (NULL != outFilePrt) {
        for (curTime = 0.0f; curTime < runTime; curTime += deltaTimeS) {
            res = rocket_math_model_2d (rocketLength, GravitationalAcceleration, curX, curY, curAlfa, curVx, curVy,
                                        curOmega, &newX, &newY, &newAlfa, &newVx, &newVy, &newOmega, deltaTimeS, mass,
                                        Fjet, Fcorrect);
            curX = newX;
            curY = newY;
            curAlfa = newAlfa;
            ...
            curOmega = newOmega;
            fprintf (outFilePrt,
                     "\n t:%6.3f X:%6.3f m Y:%6.3f m alfa:%6.3f rag Vx %6.3f m/s Vy %6.3f m/s omega %6.3f rag/s",
                     curTime, newX, newY, newAlfa, newVx, newVy, newOmega);
        }
        fclose (outFilePrt);
        res = true;
    }
