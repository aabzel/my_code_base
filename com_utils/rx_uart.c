...
#define UART_WRITER(os, uart, size) \
    ...
    generic_writer_t (os) = { { writer_putc, writer_puts}, 0, 0, 0, 0, FIFO_CHAR_INIT_WITH_IC_MUTEX((size), (os##_data), IC_MUTEX_LOG), uart_writer_transmit, (void*)(uart)};
UART_WRITERS
#else
#define UART_WRITER(os, uart, size) \
    NOINIT static char os##_data[(size)] SECTION; \
    generic_writer_t (os) = { { writer_putc, writer_puts }, 0, 0, 0, 0, FIFO_CHAR_INIT((size), (os##_data)), uart_writer_transmit, (void*)(uart)};
UART_WRITERS
#endif
#undef  UART_WRITER
...
#define UART_WRITER(os, uart, size) if (h == uart) { return &os; }
...
#define UART_READER(r, uart, string_len, queue_size, callback) if (UartHandle == r.huart) { \
    ...
    UART_READERS
#undef  UART_READER
#define UART_WRITER(os, uart, size) if (UartHandle == uart) { writer_error_callback(&os); }
    UART_WRITERS
#undef  UART_WRITER
}
#endif
...
#define UART_WRITER(os, uart, size) if (UartHandle == uart) { os.f_transmit(&os);}
    UART_WRITERS
#undef  UART_WRITER
}

void RX_UART_Recv_callback(UartHandle_t* _uart, char c) {
#define UART_READER(r, uart, string_len, queue_size, callback) if (_uart == r.huart) { uart_string_reader_rx_callback(&r, c); }
    UART_READERS
#undef  UART_READER
}

/*void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle) {
#define UART_READER(r, uart, string_len, queue_size, callback) if (UartHandle == r.huart) { uart_string_reader_rx_callback(&r, r.char_to_get); }
    UART_READERS
#undef  UART_READER
}*/
...
#define UART_READER(r, uart, string_len, queue_size, callback) \
        oprintf(stream, TABLE_LEFT "%15s " TABLE_SEPARATOR "%16" PRId64 " " TABLE_SEPARATOR "%12" PRId64 " " TABLE_SEPARATOR "%10" PRId64 " " TABLE_SEPARATOR "%10" PRId64 " " TABLE_RIGHT CRLF, #r, \
                r.total_char_count, \
                r.lost_char_count, \
...
#define GENERIC_WRITER(os) \
        oprintf(stream, TABLE_LEFT "%15s " TABLE_SEPARATOR "%16" PRId64 " " TABLE_SEPARATOR "%12" PRId64 " " TABLE_SEPARATOR "           " TABLE_SEPARATOR "%10" PRId64 " " TABLE_RIGHT CRLF, #os, \
                os.total_char_count, \
                os.lost_char_count, \
                os.error_count);
...
#ifdef SPC5
#define READER_PORT_INIT(uart) linflex_enable_recv(uart)
#endif

#define UART_WRITER(os, uart, size) sciEnableNotification(uart, SCI_FE_INT | SCI_OE_INT | SCI_PE_INT | SCI_RX_INT | SCI_TX_INT | SCI_BREAK_INT);
#define UART_READER(r, uart, string_len, queue_size, callback) \
