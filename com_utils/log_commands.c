...
static void report_log_leveles (void) {
    ...
    for (; f < ALL_FACILITY; f++) {
        rx_printf ("|%8s|%s%8s%s|" CRLF, facility2str (f), log_level_color (log_levels[f]),
...
SHELL_DEBUG_CMD_REGISTER ("log", "log_flush", "lf", cmd_log_flush,
                          "enables/disables wait for UART queue flush after every log line")
