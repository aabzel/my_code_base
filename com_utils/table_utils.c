...
static void table_row(ostream_t * row_ostream, const table_col_t row_cols[], int16_t row_col_count, const table_row_format_t * row_format);
static void table_row_hdr(ostream_t * row_hdr_ostream, const table_col_t row_hdr_cols[], int16_t row_hdr_col_count, const table_row_format_t * row_hdr_format);

static int16_t get_col_count(const table_col_t cols[]) {
    int16_t count = 0;
    while(cols[count].width > 0U) {
        count++;
    }
    return count;
}
...
void table_row_top(ostream_t * row_top_ostream, const table_col_t row_top_cols[], int16_t row_top_col_count) {
    static const table_row_format_t row_top_format = {
        TABLE_TOP_LEFT,
        TABLE_TOP_SEPARATOR,
        TABLE_TOP_RIGHT,
        TABLE_TOP_FILL
...
void table_row_separator(ostream_t * row_separator_ostream, const table_col_t row_separator_cols[], int16_t row_separator_col_count) {
    static const table_row_format_t row_separator_format = {
        TABLE_SEP_LEFT,
        TABLE_SEP_SEPARATOR,
        TABLE_SEP_RIGHT,
        TABLE_SEP_FILL
...
void table_row_bottom(ostream_t * row_bottom_ostream, const table_col_t row_bottom_cols[], int16_t row_bottom_col_count) {
    static const table_row_format_t row_bottom_format = {
        TABLE_BTM_LEFT,
        TABLE_BTM_SEPARATOR,
        TABLE_BTM_RIGHT,
        TABLE_BTM_FILL
...
void table_row_header(ostream_t * row_header_ostream, const table_col_t row_header_cols[], int16_t row_header_col_count) {
    static const table_row_format_t row_header_format = {
        TABLE_LEFT,
        TABLE_SEPARATOR,
        TABLE_RIGHT,
        TABLE_FILL
