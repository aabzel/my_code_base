...
#define XCP_LZ4_MEMORY NOINIT
    ...
    if (!not_compress) {
        int compressed_size = LZ4_compress_fast_extState(&lz4_context, (const char *) data,
                                                         lz4_data_buffer.lz4_data.data, data_size,
                                                         LZ4_DATA_BLOCK_SIZE, 1);
        if ((compressed_size != 0) && (compressed_size + 1 + sizeof(uint16_t)) < (int) data_size) {
            compressed = true;
            LOG_DEBUG(SYS, "Compress data from %" PRIu16 "to %" PRIu16 , data_size, compressed_size + 1 + (uint16_t)sizeof(uint16_t));
...
#ifdef COMPRESS_DATA
    if (compressed) {
        int decompressed_size = LZ4_decompress_safe (lz4_data_buffer.lz4_data.data, (char*) data_buffer, *data_size - sizeof(uint16_t), sizeof(data_buffer));
        ...
        if (decompressed_size != CONVERT16(lz4_data_buffer.lz4_data.size)) {
            LOG_ERROR(SYS, "LZ4 decompression size mismatch %d!=%u", decompressed_size, CONVERT16(lz4_data_buffer.lz4_data.size));
