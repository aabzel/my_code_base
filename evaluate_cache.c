        ...
        if (arr) {
            ...
            for (int i = 0; i < byte; i++) {
                ...
                if (index < byte) {
                    ...
                    printf ("\n  index %lu too big (%lu) ", index, MEM_SIZE_BYTE);
                }
            }
...
bool evaluate_cache (uint64_t byte_max) {
    ...
    snprintf (file_out_name, sizeof (file_out_name), "ram_access_time_%lu_step_%u.txt", MEM_SIZE_BYTE, SIZE_STEP);
    ...
    for (byte = 2; byte < MEM_SIZE_BYTE; byte *= 1.5) {
        ...
        if (0.0 < time_to_byte) {
            ...
            printf ("\n.");
#ifdef DEBUG_ERROR
            printf ("\n invalid  time_to_byte %f for byte %u", time_to_byte, byte);
#endif /*DEBUG_ERROR*/
        }
    }
    return true;
