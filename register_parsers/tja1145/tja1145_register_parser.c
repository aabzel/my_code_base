...
static bool parse_tja1145_main_status_register_3 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0 == (reg_val & (1 << 7))) {
        fprintf (out_file_prt, "\n    reg %02u bit  7:0  R transition to Sleep mode triggered by an SPI command",
                 reg_addr);
    }
    if (reg_val & (1 << 7)) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  7:1  R an undervoltage on VCC and/or VIO forced a transition to Sleep mode",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 6))) {
        fprintf (out_file_prt, "\n    reg %02u bit  6:0  R IC temperature below overtemperature warning threshold",
                 reg_addr);
    }
    if (reg_val & (1 << 6)) {
        fprintf (out_file_prt, "\n    reg %02u bit  6:1  R IC temperature above overtemperature warning threshold",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 5))) {
...
static bool parse_tja1145_system_event_capture_enable_register_4 (uint8_t reg_val, FILE *out_file_prt,
                                                                  uint8_t reg_addr) {
    bool res = false;
    uint16_t code = 0U;
    (void)code;
...
static bool parse_tja1145_lock_control_register_10 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0 == (reg_val & (1 << 6))) {
        fprintf (out_file_prt, "\n    reg %02u bit  6:0  R SPI write-access enabled address area 0x68 to 0x6F",
                 reg_addr);
    }
    if (reg_val & (1 << 6)) {
        fprintf (out_file_prt, "\n    reg %02u bit  6:1  R SPI write-access disabled address area 0x68 to 0x6F",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 5))) {
        fprintf (out_file_prt, "\n    reg %02u bit  5:0  R SPI write-access enabled address area 0x50 to 0x5F",
                 reg_addr);
    }
    if (reg_val & (1 << 5)) {
        fprintf (out_file_prt, "\n    reg %02u bit  5:1  R SPI write-access disabled address area 0x50 to 0x5F",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 4))) {
        fprintf (out_file_prt, "\n    reg %02u bit  4:0  R SPI write-access enabled address area 0x40 to 0x4F",
                 reg_addr);
    }
    if (reg_val & (1 << 4)) {
        fprintf (out_file_prt, "\n    reg %02u bit  4:1  R SPI write-access disabled address area 0x40 to 0x4F",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 3))) {
        fprintf (out_file_prt, "\n    reg %02u bit  3:0  R SPI write-access enabled address area 0x30 to 0x3F",
                 reg_addr);
    }
    if (reg_val & (1 << 3)) {
        fprintf (out_file_prt, "\n    reg %02u bit  3:1  R SPI write-access disabled address area 0x30 to 0x3F",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 2))) {
        fprintf (out_file_prt, "\n    reg %02u bit  2:0  R SPI write-access enabled address area 0x20 to 0x2F",
                 reg_addr);
    }
    if (reg_val & (1 << 2)) {
        fprintf (out_file_prt, "\n    reg %02u bit  2:1  R SPI write-access disabled address area 0x20 to 0x2F",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 1))) {
        fprintf (out_file_prt, "\n    reg %02u bit  1:0  R SPI write-access enabled address area 0x10 to 0x1F",
                 reg_addr);
    }
    if (reg_val & (1 << 1)) {
        fprintf (out_file_prt, "\n    reg %02u bit  1:1  R SPI write-access disabled address area 0x10 to 0x1F",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 0))) {
...
static bool parse_tja1145_can_control_register_32 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0 == (reg_val & (1 << 5))) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  5:0  RW partial networking register configuration invalid (wake-up via standard "
                 "wake-up pattern only)",
                 reg_addr);
    }
    if (reg_val & (1 << 5)) {
        fprintf (out_file_prt, "\n    reg %02u bit  5:1  RW partial networking registers configured successfully",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 4))) {
    ...
    if (1 == (0x3 & (reg_val >> 0))) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit 1-0:1 RW Active mode (when the TJA1145 is in Normal mode); VCC 90 %% undervoltage "
                 "detection active",
                 reg_addr);
    }
    if (2 == (0x3 & (reg_val >> 0))) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit 1-0:2 RW Active mode (when the TJA1145 is in Normal mode); VCC undervoltage "
                 "detection inactive; TJA1145 switches from Normal to Off mode when VBAT < Vuvd(CAN)",
                 reg_addr);
    }
    if (3 == (0x3 & (reg_val >> 0))) {
...
static bool parse_tja1145_transceiver_status_register_34 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0 == (reg_val & (1 << 6))) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  6:0  R no CAN partial networking error detected (PNFDE = 0 AND PNCOK = 1)",
                 reg_addr);
    }
    if (reg_val & (1 << 6)) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  6:1  R    CAN partial networking error detected (PNFDE = 1 OR  PNCOK = 0); "
                 "wake-up via standard wake-up pattern only",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 5))) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  5:0  R CAN partial networking configuration error detected (PNCOK = 0)",
                 reg_addr);
    }
    if (reg_val & (1 << 5)) {
        fprintf (out_file_prt, "\n    reg %02u bit  5:1  R CAN partial networking configuration OK (PNCOK = 1)",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 4))) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  4:0  R CAN partial networking oscillator not running at target frequency ",
                 reg_addr);
    }
    if (reg_val & (1 << 4)) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  4:1  R CAN partial networking oscillator running at target frequency  ",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 3))) {
    ...
    if (0 == (reg_val & (1 << 1))) {
        fprintf (out_file_prt, "\n    reg %02u bit  1:0  R VCC is above the undervoltage detection threshold",
                 reg_addr);
    }
    if (reg_val & (1 << 1)) {
        fprintf (out_file_prt, "\n    reg %02u bit  1:1  R VCC is below the undervoltage detection threshold",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 0))) {
...
static bool parse_tja1145_transceiver_event_capture_enable_register_35 (uint8_t reg_val, FILE *out_file_prt,
                                                                        uint8_t reg_addr) {
    bool res = false;
    uint16_t code = 0U;
    (void)code;
...
static bool parse_tja1145_frame_control_register_47 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (reg_val & (1 << 6)) {
        fprintf (out_file_prt, "\n    reg %02u bit  6:1  R data length code and data field are evaluated at wake-up",
                 reg_addr);
    }
    code = (0x3 & (reg_val >> 4));
...
static bool parse_tja1145_wake_status_register_75 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0 == (reg_val & (1 << 1))) {
        fprintf (out_file_prt, "\n    reg %02u bit  1:0  R voltage on WAKE pin below switching threshold (Vth(sw))",
                 reg_addr);
    }
    if (reg_val & (1 << 1)) {
        fprintf (out_file_prt, "\n    reg %02u bit  1:1  R voltage on WAKE pin above switching threshold (Vth(sw))",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 0))) {
        fprintf (out_file_prt, "\n    reg %02u bit  0:0  R reserved", reg_addr);
...
static bool parse_tja1145_wake_pin_event_capture_enable_register_76 (uint8_t reg_val, FILE *out_file_prt,
                                                                     uint8_t reg_addr) {
    bool res = false;
    uint16_t code = 0U;
    (void)code;
...
static bool parse_tja1145_system_event_status_register_97 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (reg_val & (1 << 4)) {
        fprintf (out_file_prt, "\n    reg %02u bit  4:1  RW the TJA1145 has left Off mode after battery power-on",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 3))) {
    ...
    if (reg_val & (1 << 2)) {
        fprintf (out_file_prt,
                 "\n    reg %02u bit  2:1  RW the global chip temperature has exceeded the overtemperature warning "
...
static bool parse_tja1145_transceiver_event_status_register_99 (uint8_t reg_val, FILE *out_file_prt, uint8_t reg_addr) {
    ...
    if (0 == (reg_val & (1 << 5))) {
        fprintf (out_file_prt, "\n    reg %02u bit  5:0  RW no partial networking frame detection error detected",
                 reg_addr);
    }
    if (reg_val & (1 << 5)) {
        fprintf (out_file_prt, "\n    reg %02u bit  5:1  RW partial networking frame detection error detected",
                 reg_addr);
    }
    if (0 == (reg_val & (1 << 4))) {
...
bool parse_tja1145_reg (uint8_t reg_addr, FILE *out_file_prt) {
    ...
    fprintf (out_file_prt, "\n\nReg: [%s] reg_addr: 0x%02x val: 0x%02x 0b_%s", tja1145RegMap[reg_addr].reg_name,
             reg_addr, reg_val, utoa_bin8 (reg_val));
    switch (reg_addr) {
    case 1:
        proc_reg_cnt++;
    ...
    case 126:
        ...
        break;
    default:
        fprintf (out_file_prt, "\n Lack of parser for reg %s addr 0x%x val 0x%x", tja1145_reg_2_name (reg_addr),
                 reg_addr, reg_val);
        break;
    }
    return res;
