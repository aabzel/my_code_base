...
bool stack_str_init (strStack_t **stack_instance, int des_capacity) {
    ...
    if (NULL != (*stack_instance)) {
        ((*stack_instance)->data_ptr) = (char *)malloc (des_capacity * sizeof (char *));
    ...
    if (stack_instance->index.capacity < new_des_capacity) {
        ...
        if (array) {
            if (0 < (stack_instance->index.pos)) {
                memcpy (array, stack_instance->data_ptr, sizeof (char *) * (stack_instance->index.pos));
...
bool stack_str_push (strStack_t *stack_instance, char *in_str) {
    ...
    if (stack_instance) {
        ...
        if (true == res) {
            ...
            ((stack_instance->data_ptr[(stack_instance)->index.pos])) = strdup (in_str);
