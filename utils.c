...
void init_file_name(void) {
	...
	sprintf(pemutationFile, "permutation_%d_%d_%d_%d_%d_%d.txt",
			timeinfo->tm_mday, timeinfo->tm_mon + 1, timeinfo->tm_year + 1900,
			timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);

	sprintf(kitFile, "kit_%d_%d_%d_%d_%d_%d.txt", timeinfo->tm_mday,
...
char* format_time_txt(void) {
	...
	sprintf(output, "[%d %d %d %d:%d:%d].txt", timeinfo->tm_mday,
			timeinfo->tm_mon + 1, timeinfo->tm_year + 1900, timeinfo->tm_hour,
			timeinfo->tm_min, timeinfo->tm_sec);
	return output;
}
...
double avrage_two(int val1, int val2) {
	double avagage = (double) (((double) (((double) val1) + ((double) val2))
			/ 2.0));
	return avagage;
}
...
//15 10
uint16_t extract_subval_from_16bit(uint16_t inVal, uint8_t maxBit,
		uint8_t minBit) {
	uint16_t outVal = 0;
