	...
	if ((NULL != text) && (NULL != pattern)) {
		...
		if ('\0' == (*pattern)) {
			...
			for (; '\0' != text[i]; ++i) {
				int j = 0;
				while (('\0' != text[i + j])
						&& ((tolower((int) text[i + j])
								== tolower((int) pattern[j])))) {
					++j;
				}
				if ('\0' == (pattern[j])) {
					retPtr = &text[i];
...
bool is_real_number(const char str[]) {
	...
	if (true == strtod_success) {
		...
		if (str[str_index] == '.') {
			...
			if (0 < isdigit((int32_t)(str[str_index]))) {
				while (0 < isdigit((int32_t)(str[str_index]))) {
					(void) try_dec_char_to_u8((uint8_t) str[str_index],
							&temp_value);
					number = (number * 10.0) + (double_t) temp_value;
					str_index++;
					num_digits++;
...
void detect_change(char *oldStr, char *newStr, char **oldSubStr,
		int *oldSubStringLen, char **newSubStr, int *newSubStringLen) {
	(void) *oldSubStr;
	(void) *newSubStr;
	*oldSubStringLen = 0;
	*newSubStringLen = 0;
	...
	find_diff(oldStr, oldStrLen, newStr, newStrLen, oldSubStringLen,
...
void find_diff(char *oldStr, int oldLen, char *newStr, int newLen,
		int *outOldSubStringLen, int *outNewSubStringLen, char **oldSubStr,
		char **newSubStr) {
	// printf ("\n");
	...
	if ((0 < oldLen) && (0 < newLen)) {
		if (oldStr[0] == newStr[0]) {
			find_diff(&oldStr[1], oldLen - 1, &newStr[1], newLen - 1,
					outOldSubStringLen, outNewSubStringLen, oldSubStr,
					newSubStr);
		} else {
			if (oldStr[oldLen - 1] == newStr[newLen - 1]) {
				// oldStr [oldLen - 1] = '\0';
				// newStr [newLen - 1] = '\0';
				find_diff(oldStr, oldLen - 1, newStr, newLen - 1,
						outOldSubStringLen, outNewSubStringLen, oldSubStr,
						newSubStr);
			} else {
			...
			if (1 == valLen) {
				...
				if (true == is_hex_number(inStr[i])) {
					...
					if ((' ' == inStr[i]) || (']' == inStr[i])) {
						...
						if (4 == valLen) {
							res = try_strl2uint8_hex(&inStr[startIndex + 2], 2,
									val8b);
							return res;
						}
						catch = 0;
			...
			if (1 == valLen) {
				...
				if (true == is_hex_number(inStr[i])) {
					...
					if (' ' == inStr[i]) {
						hexValCnt++;
						if (6 == valLen) {
							res = try_strl2uint16_hex(&inStr[startIndex + 2], 4,
									val16b);
							return res;
						}
						catch = 0;
			...
			if (1 == valLen) {
				...
				if (true == is_hex_number(inStr[i])) {
					...
					if (' ' == inStr[i]) {
						hexValCnt++;
						if (10 == valLen) {
							res = try_strl2uint32_hex(&inStr[startIndex + 2], 8,
									val32b);
							return res;
						}
						catch = 0;
...
char* mac_to_str(uint8_t *mac_addr) {
	...
	snprintf((char*) macStrOut, sizeof(macStrOut),
...
char* ip_to_str(uint8_t *ip_addr) {
	...
	snprintf((char*) ipStrOut, sizeof(ipStrOut), "%02d.%02d.%02d.%02d",
			ip_addr[3], ip_addr[2], ip_addr[1], ip_addr[0]);
	return ipStrOut;
}

bool parse_substr_name(const char *const inStr, uint16_t inStrLen,
		char *const outStr, char openBracket, char closedBracket) {
...
bool parse_mac(char *inStr, uint16_t inStrLen, uint8_t *outMacAddr) {
	...
	if (strlen("MAC") < inStrLen) {
		...
		if (NULL != serialStartPtr) {
			res = try_str2mac((const char*) (serialStartPtr + strlen("MAC ")),
					outMacAddr);
		} else {
...
bool parse_ip(char *inStr, uint16_t inStrLen, uint32_t *outIpAddr) {
	...
	if (strlen("IP ") < inStrLen) {
		...
		if (NULL != serialStartPtr) {
			...
			res = try_strl2ipv4((const char*) (serialStartPtr + strlen("IP ")),
					IpLen, outIpAddr);
		} else {
...
#endif
			hexValLen = calc_hex_val_len(
					(serialStartPtr + strlen("Serial") + 1));
#if DEBUG_PARSE_SERIAL
            printf ("\n hexValLen[%d]", hexValLen);
#endif
			res = try_strl2uint64(
					(const char*) (serialStartPtr + strlen("Serial") + 1),
					hexValLen, &localSerial64bNumber);
			if (true == res) {
