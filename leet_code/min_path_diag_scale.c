...
static float min_path_diag_scale_sum (int *grid, int numLine, int numColumn, int curCellx, int curCelly,
                                      float curPrevSumm, bool isDiagEntry);

static float min_path_diag_scale (int *grid, int numLine, int numColumn, int curCellx, int curCelly, float curPrevSumm,
                                  float minSum, Cell_t *const dotArr, int lenOfCurrPath, Cell_t **finalPath,
                                  int *lenOfFinalPath, bool isDiag);
...
static float min_path_diag_scale_sum (int *grid, int numLine, int numColumn, int curCellx, int curCelly,
    ...
    if (curCellx < (numColumn - 1)) {
        if (curCelly < (numLine - 1)) {
            ...
            minPathSum1 = min_path_diag_scale_sum (grid, numLine, numColumn, curCellx + 1, curCelly + 1, curTotalSum1,
                                                   DIAG_ENTRY);

            curAddSumOut = (((float)curCellVal) / (2.0f));
            curTotalSum2 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum2 = min_path_diag_scale_sum (grid, numLine, numColumn, curCellx, curCelly + 1, curTotalSum2,
                                                   STRAIGHT_ENTRY);

            curAddSumOut = (((float)curCellVal) / (2.0f));
            curTotalSum3 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum3 = min_path_diag_scale_sum (grid, numLine, numColumn, curCellx + 1, curCelly, curTotalSum3,
                                                   STRAIGHT_ENTRY);
            ...
            curAddSumOut = (((float)curCellVal) / (2.0f));
            curTotalSum1 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum = min_path_diag_scale_sum (grid, numLine, numColumn, curCellx + 1, curCelly, curTotalSum1,
                                                  STRAIGHT_ENTRY);
        }
    } else {
        ...
        if (curCelly < (numLine - 1)) {
            curAddSumOut = (((float)curCellVal) / (2.0f));
            curTotalSum1 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum = min_path_diag_scale_sum (grid, numLine, numColumn, curCellx, curCelly + 1, curTotalSum1,
                                                  STRAIGHT_ENTRY);
        } else {
            // curCellx=numColumn-1
            // curCelly=numLine-1
    ...
    *minPathArr = NULL;
    ...
    if ((0 == *minPathLen) && (NULL == *minPathArr)) {
        ...
        if (currPath1) {
            ...
            min_path_diag_scale ((int *)grid, numLine, numColumn, x + 1, y + 1, outSumm, minSum, currPath1,
                                 lenOfCurrPath, minPathArr, minPathLen, true);
        }
    }
    ...
    if ((0 == *minPathLen) && (NULL == *minPathArr)) {
        ...
        if (currPath2) {
            ...
            min_path_diag_scale ((int *)grid, numLine, numColumn, x + 1, y, outSumm, minSum, currPath2, lenOfCurrPath,
                                 minPathArr, minPathLen, false);
        }
    }
    ...
    if ((0 == *minPathLen) && (NULL == *minPathArr)) {
        ...
        if (currPath3) {
            ...
            min_path_diag_scale ((int *)grid, numLine, numColumn, x, y + 1, outSumm, minSum, currPath3, lenOfCurrPath,
                                 minPathArr, minPathLen, false);
        }
    }
}
...
static float min_path_diag_scale (int *grid, int numLine, int numColumn, int curCellx, int curCelly, float curPrevSumm,
    ...
    if (curCellx < (numColumn - 1)) {
        if (curCelly < (numLine - 1)) {
            ...
            minPathSum1 = min_path_diag_scale (grid, numLine, numColumn, curCellx + 1, curCelly + 1, curTotalSum1,
                                               minSum, locArr, lenOfCurrPath + 1, finalPath, lenOfFinalPath, true);

            curAddSumOut = ((float)curCellVal) / (2.0f);
            float curTotalSum2 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum2 = min_path_diag_scale (grid, numLine, numColumn, curCellx, curCelly + 1, curTotalSum2, minSum,
                                               locArr, lenOfCurrPath + 1, finalPath, lenOfFinalPath, false);

            curAddSumOut = ((float)curCellVal) / (2.0f);
            float curTotalSum3 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum3 = min_path_diag_scale (grid, numLine, numColumn, curCellx + 1, curCelly, curTotalSum3, minSum,
                                               locArr, lenOfCurrPath + 1, finalPath, lenOfFinalPath, false);
            ...
            curAddSumOut = ((float)curCellVal) / (2.0f);
            float curTotalSum1 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum = min_path_diag_scale (grid, numLine, numColumn, curCellx + 1, curCelly, curTotalSum1, minSum,
                                              locArr, lenOfCurrPath + 1, finalPath, lenOfFinalPath, false);
        }
    } else {
        // curCellx=numColumn-1
        if (curCelly < (numLine - 1)) {
            curAddSumOut = ((float)curCellVal) / (2.0f);
            float curTotalSum1 = curPrevSumm + curAddSumIn + curAddSumOut;
            minPathSum = min_path_diag_scale (grid, numLine, numColumn, curCellx, curCelly + 1, curTotalSum1, minSum,
                                              locArr, lenOfCurrPath + 1, finalPath, lenOfFinalPath, false);
        } else {
            // curCellx=numColumn-1
            // curCelly=numLine-1
