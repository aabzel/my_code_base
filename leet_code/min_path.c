...
static int min_path (int *grid, int numLine, int numColumn, int curCellx, int curCelly, int curPrevSumm, int minSum,
                     Cell_t *const dotArr, int lenOfCurrPath, Cell_t **finalPath, int *lenOfFinalPath);

static int min_path_diag (int *grid, int numLine, int numColumn, int curCellx, int curCelly, int curPrevSumm,
                          int minSum, Cell_t *const dotArr, int lenOfCurrPath, Cell_t **finalPath, int *lenOfFinalPath);
...
/*«аписывать в файл все пути а потом произвести его синтаксический разбор и выбрать тот, что дошел до конца.
 * (медленно)*/
    ...
    *minPathArr = NULL;
    ...
    if ((0 == *minPathLen) && (NULL == *minPathArr)) {
        ...
        if (currPath1) {
            ...
            min_path_diag (grid, numLine, numColumn, x + 1, y + 1, curSum, minSum, currPath1, lenOfCurrPath, minPathArr,
                           minPathLen);
        }
    }
    ...
    if ((0 == *minPathLen) && (NULL == *minPathArr)) {
        ...
        if (currPath2) {
            ...
            min_path_diag (grid, numLine, numColumn, x + 1, y, curSum, minSum, currPath2, lenOfCurrPath, minPathArr,
                           minPathLen);
        }
    }
    ...
    if ((0 == *minPathLen) && (NULL == *minPathArr)) {
        ...
        if (currPath3) {
            ...
            min_path_diag (grid, numLine, numColumn, x, y + 1, curSum, minSum, currPath3, lenOfCurrPath, minPathArr,
                           minPathLen);
        }
    }
...
static int min_path (int *grid, int numLine, int numColumn, int curCellx, int curCelly, int curPrevSumm, int minSum,
    ...
    if (curCellx < (numColumn - 1)) {
        if (curCelly < (numLine - 1)) {
            ...
            minPathSum1 = min_path (grid, numLine, numColumn, curCellx, curCelly + 1, curOutSum, minSum, locArr,
                                    lenOfCurrPath + 1, finalPath, lenOfFinalPath);
            minPathSum2 = min_path (grid, numLine, numColumn, curCellx + 1, curCelly, curOutSum, minSum, locArr,
                                    lenOfCurrPath + 1, finalPath, lenOfFinalPath);
            minPathSum = min (minPathSum1, minPathSum2);
        } else {
            // curCelly=numLine-1
            curOutSum = curPrevSumm + curCellVal;
            minPathSum = min_path (grid, numLine, numColumn, curCellx + 1, curCelly, curOutSum, minSum, locArr,
                                   lenOfCurrPath + 1, finalPath, lenOfFinalPath);
        }
    } else {
        ...
        if (curCelly < (numLine - 1)) {
            curOutSum = curPrevSumm + curCellVal;
            minPathSum = min_path (grid, numLine, numColumn, curCellx, curCelly + 1, curOutSum, minSum, locArr,
                                   lenOfCurrPath + 1, finalPath, lenOfFinalPath);
        } else {
            // curCellx=numColumn-1
            // curCelly=numLine-1
...
static int min_path_diag (int *grid, int numLine, int numColumn, int curCellx, int curCelly, int curPrevSumm,
                          int minSum, Cell_t *const dotArr, int lenOfCurrPath, Cell_t **finalPath,
                          int *lenOfFinalPath) {
    static int callDeep = 1;
    callDeep++;
    int curOutSum = 0;
    ...
    if (curCellx < (numColumn - 1)) {
        if (curCelly < (numLine - 1)) {
            curOutSum = curPrevSumm + curCellVal;
            minPathSum1 = min_path_diag (grid, numLine, numColumn, curCellx + 1, curCelly + 1, curOutSum, minSum,
                                         locArr, lenOfCurrPath + 1, finalPath, lenOfFinalPath);
            minPathSum2 = min_path_diag (grid, numLine, numColumn, curCellx, curCelly + 1, curOutSum, minSum, locArr,
                                         lenOfCurrPath + 1, finalPath, lenOfFinalPath);
            minPathSum3 = min_path_diag (grid, numLine, numColumn, curCellx + 1, curCelly, curOutSum, minSum, locArr,
                                         lenOfCurrPath + 1, finalPath, lenOfFinalPath);
            minPathSum = min3 (minPathSum1, minPathSum2, minPathSum3);
        } else {
            // curCelly=numLine-1
            curOutSum = curPrevSumm + curCellVal;
            minPathSum = min_path_diag (grid, numLine, numColumn, curCellx + 1, curCelly, curOutSum, minSum, locArr,
                                        lenOfCurrPath + 1, finalPath, lenOfFinalPath);
        }
    } else {
        // curCellx=numColumn-1
        if (curCelly < (numLine - 1)) {
            curOutSum = curPrevSumm + curCellVal;
            minPathSum = min_path_diag (grid, numLine, numColumn, curCellx, curCelly + 1, curOutSum, minSum, locArr,
                                        lenOfCurrPath + 1, finalPath, lenOfFinalPath);
        } else {
            // curCellx=numColumn-1
            // curCelly=numLine-1
...
bool save_map_path_as_dot (char *fileName, int *grid, int numLine, int numColumn, Cell_t *const arrayOfDot,
                           int sizeOfPath) {
    bool res = false;
    FILE *stream = NULL;
    stream = fopen (fileName, "w");
