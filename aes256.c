...
uint8_t R_Con[30] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xc0, 0xab, 0x4d, 0x9a,
    0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a, 0xd4, 0xb3, 0x7d, 0xfa, 0xef, 0xc5, 0x91,
};

void AES256MainECB (uint8_t *key, uint8_t *data, uint32_t datalen, uint8_t *returndata, uint32_t *returnlen,
                    bool encrypt) {
...
void AES256MainCBC (uint8_t *key, uint8_t *iv, uint8_t *data, uint32_t datalen, uint8_t *returndata,
                    uint32_t *returnlen, bool encrypt) {
...
void AES256MainCTR (uint8_t *key, uint8_t *data, uint8_t *iv, uint32_t datalen, uint8_t *returndata,
                    uint32_t *returnlen, bool encrypt) {
...
char *_Print_State (AES256_State state) {
    ...
    sprintf (buffer, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", state.x[0].x[0],
             state.x[0].x[1], state.x[0].x[2], state.x[0].x[3], state.x[1].x[0], state.x[1].x[1], state.x[1].x[2],
             state.x[1].x[3], state.x[2].x[0], state.x[2].x[1], state.x[2].x[2], state.x[2].x[3], state.x[3].x[0],
             state.x[3].x[1], state.x[3].x[2], state.x[3].x[3]);
    return buffer;
}
