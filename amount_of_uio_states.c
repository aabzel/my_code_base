...
char *uioPulls[] = {"PullUp", "PullDown", "PullNone"};

char *uioShutDownStates[] = {"ShutDownOff", "ShutDownOn"};

char *uioModes[] = {"lowSide", "highSide", "H-Bridge", "input", "Vref", "Disabled"};

char *uioLoadDet[] = {"LoadDet_On", "LoadDet_Off"};

char *bridModes[] = {"Free", "Fwd", "Rev", "Break"};

char *dutyModes[] = {"duty=0%", "0%<duty<100%", "duty=100%"};

bool save_the_amount_of_bridge_cfg (void) { return true; }
                        ...
                        for (int curLd = 0; curLd < amountOfLdStated; curLd++) {
                            if (0 == strcmp (uioModes[curMode], "H-Bridge")) {
                                for (int curBrMode = 0; curBrMode < amountOfBridgeModes; curBrMode++) {
                                    fprintf (stream, "\n%d: %s %s %s %s %s  %s", amountOfStates, uioModes[curMode],
                                             bridModes[curBrMode], uioPulls[curPullMode], uioLoadDet[curLd],
                                             dutyModes[curDutyMode], uioShutDownStates[curShutDowmModes]);
                                    amountOfStates++;
                                }
                            } else {
                                fprintf (stream, "\n%d: %s %s %s %s  %s", amountOfStates, uioModes[curMode],
                                         uioPulls[curPullMode], uioLoadDet[curLd], dutyModes[curDutyMode],
