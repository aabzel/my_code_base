...
static diag_page_rec_t index_rec =
    { DIAG_PAGE_INDEX, "Index", &diag_page_index, NULL };
...
bool diag_page_add (diag_page_rec_t *rec__dpa) {
    ...
    while ((exit_flag == false) && (curr_rec != NULL)) {
        if (curr_rec == rec__dpa) {
            LOG_ERROR (
                SYS,
                "diag_rec %s is already added",
                rec__dpa->diag_rec_name
            );
            success__dpa = false;
            exit_flag = true;
        }
        if (curr_rec->diag_rec_id == rec__dpa->diag_rec_id) {
            LOG_ERROR (
                SYS,
                "diag_recs %s and %s with same id %u",
                curr_rec->diag_rec_name,
                rec__dpa->diag_rec_name,
