...
static int get_ldnumber(const TCHAR **path /* Pointer to pointer to the path name */
) {
	const TCHAR *tp, *tt;
	UINT i;
	int vol = -1;
...
DWORD sect /* Sector# (lba) to check if it is an FAT boot record or not */
	...
	rx_printf("%s() sec %u" CRLF, __FUNCTION__, sect);
	fs->wflag = 0;
...
BYTE wmode /* !=0: Check write protection for write access */
	...
	*rfs = 0;
	...
	LOG_NOTICE(SYS, "%s() Line %u", __FUNCTION__, __LINE__);
	wait_in_loop_ms(10);
	...
	*rfs = fs; /* Return pointer to the file system object */
	...
	if (fmt) {
		LOG_NOTICE(SYS, "%s() fmt %u", __FUNCTION__, fmt);
	...
	clear_lock(fs);
#endif
	LOG_NOTICE(SYS, "%s() done", __FUNCTION__);
	return FR_OK;
}
...
BYTE opt /* 0:Do not mount (delayed mount), 1:Mount immediately */
	...
	vol = get_ldnumber(&rp);
	if (vol < 0) {
		LOG_NOTICE(SYS, "%s() error", __FUNCTION__);
		return FR_INVALID_DRIVE;
	} else {
		LOG_NOTICE(SYS, "get_ldnumber ok");
	}
